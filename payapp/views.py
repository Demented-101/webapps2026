...
def home(request):
    ...
    if request.user.is_authenticated:
        # get user balance
        user_balance = models.Balance.objects.get(name__username=request.user.username).balance

        # has unsent transaction
        if len(get_open_requests(request.user.username)) > 0: has_unsent_request = True

    return render(request, "webapps2026/home.html", context={"user_balance":user_balance, "has_unsent_requests":has_unsent_request})
...
def make_transaction(request):
    if request.method == 'POST':
        ...
        if form.is_valid() and request.user.is_authenticated:
            ...
            if src_balance.balance < amount_to_transfer: return render_form(request, form, "payapp/FundTransaction.html", error = "Insufficient funds.")
...
def request_transaction(request):
    if request.method == "POST":
        ...
        if form.is_valid() and request.user.is_authenticated:
            ...
            if not models.Balance.objects.filter(name__username=dst_username).exists() and dst_username != src_username:
...
def view_transaction_requests(request, ignore_post=False):
    if request.method == "POST" and not ignore_post:
        if request.user.is_authenticated:
            ...
            transaction_request = models.BalanceTransactionRequest.objects.get(id=request_id)
    ...
    if request.user.is_authenticated:
        ...
        set_a = models.BalanceTransactionRequest.objects.filter(to_username=username)
        set_b = models.BalanceTransactionRequest.objects.filter(from_username=username)
        all_requests = (set_a | set_b).order_by("-created_at").values()

        # get user balance
        balance = models.Balance.objects.get(name__username=username).balance

    return render(request, "payapp/TransactionRequests.html", context={"open_requests":open_requests, "all_requests":all_requests, "balance":balance})

@csrf_exempt
def get_open_requests(username):
    transaction_requests = models.BalanceTransactionRequest.objects.filter(to_username=username) & \
                           models.BalanceTransactionRequest.objects.filter(open=True)
